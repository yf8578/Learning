...
def single_task(code_dir, ref, out_path, fqtype, cut_f2, sample_name, fq1, fq2=None):
    cmd = f"{software.python3} -u {code_dir}/pipeline.py  --scr {code_dir} -r {ref} --fqtype {fqtype} -s {sample_name} --fq1 {fq1} " \
          f"--fq2 {fq2} -o {out_path} --cut_r2 {cut_f2}"
    # print(cmd)
...
def main():
    ...
    if "PE" in opt.fqtype:
        for idx, row in sample_info.iterrows():
            p.apply_async(single_task, args=(script_dir, opt.ref, out_path, opt.fqtype, opt.cut_r2, row[0], row[1], row[2],))
    else:
        for idx, row in sample_info.iterrows():
            # print(script_dir, opt.ref, out_path, opt.fqtype, row[0], row[1])
            # single_task(script_dir, opt.ref, out_path, opt.fqtype, row[0], row[1])
            p.apply_async(single_task, args=(script_dir, opt.ref, out_path, opt.fqtype, opt.cut_r2, row[0], row[1]))
    p.close()
    p.join()
