...
def build_primer_index(path: str, primer_file: str, adapter_flag: bool):
    ...
    try:
        if '.xlsx' in primer_file:
            primer = pd.read_excel(primer_file)
        else:
            seps = ',' if '.csv' in primer_file else '\s+'
            primer = pd.read_csv(primer_file, sep=seps)
    except IOError as e:
        logging.error(e)
        sys.exit(2)
    if adapter_flag:
    ...
    with open('primer.fa', 'w') as pw, open('primers_info.txt', 'w') as f:
        ...
        for idx, row in primer.iterrows():
            ...
            if row.primer1 not in primer_f.keys():
                ...
                pw.write(update_seqs(row.name_id, row.primer1, 'F'))
            ...
            if row.primer2 not in primer_r.keys():
                ...
                pw.write(update_seqs(row.name_id, row.primer2, 'R'))
...
def replace_base_fa(seqs, merge_base='Y'):
    # 替换兼并碱基可能生成的新序列
    bases = 'CT' if merge_base =='Y' else 'GA'
    seqs_lst = seqs.split(merge_base)
    merge_lst = [ list(x) for x in it.product(bases, repeat=len(seqs_lst)-1)]
    for b in merge_lst:
        new_seq = list("".join(x) for x in list(zip(seqs_lst[:-1] ,b))) + seqs_lst[-1:]
        yield "".join(new_seq)


def update_seqs(name_id, primer_seq, strand='F'):
    # 补充潜在的引物序列
    res = ''
    if 'Y' in primer_seq:
        merge_base = 'Y'
    elif 'R'in primer_seq:
        merge_base = 'R'
    else:
        return f">{name_id}-{strand}\n{primer_seq}\n"
    for idx, prim in enumerate(replace_base_fa(primer_seq, merge_base)):
        res += f'>{name_id}-{strand}.{idx + 1}\n{prim}\n'
    return res
...
def get_opt():
    ...
    parser.add_argument('--amp_min_len', type=int, required=False, default=100,
                        help="[optional] The minimum amplicon size, default=100. [int]")
    parser.add_argument('--vcf', type=int, choices=[0, 1], required=False, default=0,
                        help="[optional] choose to output VCF file, default=0. [0, 1]")
    ...
    parser.add_argument('--skip_all', type=int, choices=[0, 1], default=0,
                        help='[optional] choose to skip all process, just merge all result, default=0. [0, 1]')

    options = parser.parse_args()

    return options

def load_depth_df(path, sample_name, depth_name, is_first=None):
    try:
        amp_depth = pd.read_csv(f"{path}/{sample_name}/{depth_name}", sep='\s+', header=None)
        amp_depth.columns = ['chr', 'start', 'end', 'name', f'{sample_name}']
        if is_first:
            return amp_depth[['name', 'chr', 'start', 'end', f'{sample_name}']]
        else:
            return amp_depth[f'{sample_name}']
    except:
        logging.info(f"No found {path}/{sample_name}/{depth_name}")
        return None

def load_base_depth_df(path, sample_name, depth_name, is_first=None):
    try:
        amp_depth = pd.read_csv(f"{path}/{sample_name}/{depth_name}", sep='\s+', header=None)
        amp_depth.columns = ['chr', 'pos', f'{sample_name}']
        if is_first:
            return amp_depth
        else:
            return amp_depth[f'{sample_name}']
...
def load_summary(path, sample_name):
    try:
        data = pd.read_csv(f"{path}/{sample_name}/summary.csv", sep='\s+')
        return data
    except:
        logging.info(f"No found {path}/{sample_name}/summary.csv")
        return None

def merge_excel_result(data, sample_name, save_file):
    if not os.path.exists(save_file):
        data.to_excel(save_file, sheet_name=sample_name, index=False)
    else:
        book = load_workbook(save_file)
        write = pd.ExcelWriter(save_file, engine='openpyxl')
        write.book = book
        data.to_excel(write, sheet_name=sample_name, index=False)
        write.save()


def single_task(code_dir, ref, out_path, fqtype, cut_f2, amp_min_len, vcf_flag, sample_name, fq1, fq2=None):
    cmd = f"{software.python3} -u {code_dir}/pipeline.py  --scr {code_dir} -r {ref} --fqtype {fqtype} -s {sample_name} --fq1 {fq1} " \
          f"--fq2 {fq2} --amp_min_len  {amp_min_len} --vcf {vcf_flag} -o {out_path} --cut_r2 {cut_f2}"
    # print(cmd)
    pipe.run_shell(cmd, f'{sample_name} start')
...
def main():
    ...
    try:
        ...
        raise e
    if not opt.skip_all:
        process = 6 if len(sample_info) > 6 else  len(sample_info)
        p = Pool(processes=process)
        if "PE" in opt.fqtype:
            for idx, row in sample_info.iterrows():
                p.apply_async(single_task, args=(script_dir, opt.ref, out_path, opt.fqtype, opt.cut_r2, opt.amp_min_len, opt.vcf, row[0], row[1], row[2],))
        else:
            for idx, row in sample_info.iterrows():
                # print(script_dir, opt.ref, out_path, opt.fqtype, row[0], row[1])
                # single_task(script_dir, opt.ref, out_path, opt.fqtype, row[0], row[1])
                p.apply_async(single_task, args=(script_dir, opt.ref, out_path, opt.fqtype, opt.cut_r2, opt.amp_min_len, opt.vcf, row[0], row[1]))
        p.close()
        p.join()
    name_lst = sample_info.sample_name.tolist()
    amp_depth = load_depth_df(out_path, name_lst[0], f'{name_lst[0]}.depth.txt', is_first=True)
    amp_uniq_depth = load_depth_df(out_path, name_lst[0], f'{name_lst[0]}_uniq.depth.txt', is_first=True)
    base_depth = load_base_depth_df(out_path, name_lst[0], f'{name_lst[0]}_base.depth.txt', is_first=True)
    summary_res = load_summary(out_path, name_lst[0])
    if os.path.exists(f"{out_path}/MultiPcr_efficiency.xlsx"):
        os.system(f'rm {out_path}/MultiPcr_efficiency.xlsx')
    if os.path.exists(f"{out_path}/MultiPcr_unexpected.xlsx"):
        os.system(f'rm {out_path}/MultiPcr_unexpected.xlsx')
    if os.path.exists(f"{out_path}/MultiPcr_summary_depth.xlsx"):
        os.system(f'rm {out_path}/MultiPcr_summary_depth.xlsx')
    for idx, name in enumerate(name_lst):
        eff = pd.read_excel(f"{out_path}/{name}/{name}_efficiency.xlsx")
        merge_excel_result(eff, name, f"{out_path}/MultiPcr_efficiency.xlsx")
        unex = pd.read_excel(f"{out_path}/{name}/{name}_unexpected.xlsx")
        merge_excel_result(unex, name, f"{out_path}/MultiPcr_unexpected.xlsx")
        if idx > 0:
            amp_depth[f'{name}'] = load_depth_df(out_path, name, f'{name}.depth.txt')
            amp_uniq_depth[f'{name}'] = load_depth_df(out_path, name, f'{name}_uniq.depth.txt')
            base_depth[f'{name}'] = load_base_depth_df(out_path, name, f'{name}_base.depth.txt')
            summary = load_summary(out_path, name)
            summary_res = pd.concat([summary_res, summary])

    merge_excel_result(summary_res, 'summary', f"{out_path}/MultiPcr_summary_depth.xlsx")
    merge_excel_result(amp_depth, 'amp_region_depth', f"{out_path}/MultiPcr_summary_depth.xlsx")
    merge_excel_result(amp_uniq_depth, "amp_unique_region_depth", f"{out_path}/MultiPcr_summary_depth.xlsx")
    merge_excel_result(base_depth, 'base_depth', f"{out_path}/MultiPcr_summary_depth.xlsx")
